<?php
/**
 * Archive template for Workshop CPT
 * WordPress loads this automatically at /workshop/ (CPT archive URL)
 */
get_header();
...
foreach ($wp_posts as $post) {
    ...
    $slot_agg = $wpdb->get_row($wpdb->prepare(
        "SELECT
            COALESCE(SUM(b.num_seats), 0) AS total_booked,
            COALESCE(SUM(s.total_seats), 0) AS total_capacity,
            COUNT(DISTINCT s.id)            AS upcoming_slots
         FROM {$ts} s
         LEFT JOIN {$tb} b
                ON b.slot_id = s.id AND b.status NOT IN ('cancelled')
         WHERE s.workshop_id = %d
